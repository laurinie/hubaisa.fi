...
const Navigation = (props) => {
  ...
  return (
    <nav>
       ...
       {nav&&nav.links.map(p=>(
           p.slug==="/"?
           <Link  to={p.slug}><li className={window.location.pathname===`${p.slug}`?"selected":""}>{nav&&nav.title}</li></Link>:
           <Link  to={p.slug}><li className={window.location.pathname===`/${p.slug}`?"selected":""}>{p.title}</li></Link>
       ))}
       </ul>
